...
public:
    Wrecking_Ball()
    {
        ...
        d2Body *floor = m_world->CreateBody(d2BoxShape(screenWidth, 10.0f), {screenWidth / 2.0f, screenHeight - 5.0f}, 0.0f);
        ...
        for (int col = 0; col < numRows; col++)
        {
            for (int row = 0; row < col; row++)
            {
                ...
                d2Body *box = m_world->CreateBody(d2BoxShape(50, 50), (d2Vec2) {x, y}, mass);
        ...
        d2Body *anchor = m_world->CreateBody(d2BoxShape(5.f, 5.f),
        ...
        d2Body *wreckingBall = m_world->CreateBody(d2CircleShape(ballRadius), (d2Vec2) {x, y}, 100.0f);

        // Create distance constraint between the anchor and the wrecking ball
        m_world->CreateJoint(anchor, wreckingBall, anchor->GetPosition());
    }
    ...
    Input() override
    {
        if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON))
        {
            ...
            selectedBody = m_world->broadphase->Pick((d2Vec2) {mousePos.x, mousePos.y});
        }
